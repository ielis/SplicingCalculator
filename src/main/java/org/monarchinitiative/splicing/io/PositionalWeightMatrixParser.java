...
public class PositionalWeightMatrixParser {
    ...
    /**
     * Parse Yaml content of provided <code>matrixFile</code>, create PWM representations ({@link PositionWeightMatrix}) and store
     * in a {@link Map} by their names.
     *
     * @param matrixFile {@link File} with PWM definitions in Yaml format as described in {@link PositionWeightMatrix} class
     *                   description
     * @throws IOException if error occurs during Yaml decoding
     */
    public PositionalWeightMatrixParser(File matrixFile) throws IOException {
        try (InputStream is = new FileInputStream(matrixFile)) {
            Map<String, PositionWeightMatrix> matrixMap = parseAll(is);
            this.donorMatrix = mapToDoubleMatrix(matrixMap.get(DONOR_M_NAME));
            this.acceptorMatrix = mapToDoubleMatrix(matrixMap.get(ACCEPTOR_M_NAME));
        }
